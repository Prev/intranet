...
var checkIdTimeout;
var FILTERED_ID = ["admin", "administrator", "root", "webmaster", "master", "tester", "dimigo"];
...
window.onload = function () {
	...
	inputId.onkeyup = function () {
		...
		if (inputId.value.length == 0) {
			inputIdWarning.style.color = "#d02626"
			inputIdWarning.innerHTML = '아이디를 입력해 주세요.';
		}else if (inputId.value.match(/[^a-z0-9_-]+/g)) {
			inputIdWarning.style.color = "#d02626"
			inputIdWarning.innerHTML = '사용 할 수 없는 아이디입니다.';
	...
	inputPw.onkeyup= function () {
		if (inputPw.value.length == 0) {
			...
			inputPwWarning.innerHTML = '비밀번호를 입력해 주세요.';
		}
...
function checkId() {
	$.get(getUrl('signup', 'procCheckInputId', 'input_id='+inputId.value), function (result) {
