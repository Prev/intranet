...
<?php
	...
	class BoardCGIController extends BoardController {
		...
		public function procSaveArticle() {
			...
			if (isset($_POST['parent_no'])) {
				$boardId = $this->model->getParentArticleBoardId($_POST['parent_no']);
				$parentArticleData = $this->model->getArticleData($_POST['parent_no']);
				...
				if (!$parentArticleData->allow_reply) {
					goBack(array(
						'en' => 'You can\'t write reply to this post',
						'ko' => '답글을 달 수 없는 게시글입니다.'
					), true);
					return;
				}
			}
			...
			$record = $this->model->insertNewArtile(
				...
				$isNotice ? 1 : 0,
				evalCheckbox($_POST['allow_comment']) ? 1 : 0,
				evalCheckbox($_POST['allow_reply']) ? 1 : 0,
				intval($_POST['parent_no']),
				$_POST['category']
			);
